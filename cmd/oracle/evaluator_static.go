...
// genesisAxiom is a local copy of the axiom schema (mirrors x/knowledge/types.GenesisAxiom).
...
// parseAxioms unmarshals the embedded JSON into a slice of genesisAxiom.
func parseAxioms(data []byte) ([]genesisAxiom, error) {
	var axioms []genesisAxiom
	if err := json.Unmarshal(data, &axioms); err != nil {
		return nil, fmt.Errorf("unmarshal genesis axioms: %w", err)
	}
	return axioms, nil
}
...
// StaticEvaluator checks claims against genesis axioms embedded in the binary.
...
func NewStaticEvaluator() (*StaticEvaluator, error) {
	raw, err := parseAxioms(genesisAxiomsJSON)
	if err != nil {
