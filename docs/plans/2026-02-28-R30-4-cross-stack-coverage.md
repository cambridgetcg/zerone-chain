# R30-4: Cross-Stack Integration Coverage — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add cross-stack integration tests that exercise multi-feature interactions when R28/R29 features fire together.

**Architecture:** Direct state injection via keeper APIs into a full app TestHarness (matching existing `adaptive_test.go` pattern). Four test functions covering the full ecosystem cycle, adversarial edge cases, BeginBlocker/EndBlocker ordering safety, and genesis round-trip.

**Tech Stack:** Go test, testify/require, Cosmos SDK test harness with in-memory DB

---

### Task 1: Extend TestHarness with Missing Keepers

**Files:**
- Modify: `tests/cross_stack/harness_test.go`

**Step 1: Add imports and keeper fields**
...
Add these fields to the `TestHarness` struct (after `DisputesKeeper`):
...
Wire them in `NewTestHarness` (after `DisputesKeeper` assignment):
...
Run: `go test ./tests/cross_stack/... -v -count=1 -timeout 120s`
Expected: all existing tests pass

**Step 4: Commit**

```bash
git add tests/cross_stack/harness_test.go
git commit -m "test(cross_stack): extend harness with capture_defense, partnerships, discovery keepers"
```

---

### Task 2: TestR29_FullEcosystemCycle

**Files:**
- Create: `tests/cross_stack/r29_integration_test.go`

**Step 1: Create the test file with the full ecosystem cycle test**
...
Run: `go test ./tests/cross_stack/... -run TestR29_FullEcosystemCycle -v -count=1 -timeout 120s`
Expected: PASS

**Step 4: Commit**

```bash
git add tests/cross_stack/r29_integration_test.go
git commit -m "test(cross_stack): add R29 full ecosystem cycle integration test"
```

---

### Task 3: TestR29_AdversarialInteractions

**Files:**
- Modify: `tests/cross_stack/r29_integration_test.go`

**Step 1: Add the adversarial interactions test**

Append to `r29_integration_test.go`:
...
Run: `go test ./tests/cross_stack/... -run TestR29_AdversarialInteractions -v -count=1 -timeout 120s`
...
```bash
git add tests/cross_stack/r29_integration_test.go
git commit -m "test(cross_stack): add R29 adversarial interactions test"
```

---

### Task 4: TestBlockerOrdering_NoPanic

**Files:**
- Modify: `tests/cross_stack/r29_integration_test.go`

**Step 1: Add the blocker ordering test**
...
Run: `go test ./tests/cross_stack/... -run TestBlockerOrdering_NoPanic -v -count=1 -timeout 300s`
Expected: PASS (may take a while due to 1000 blocks)

**Step 3: Commit**

```bash
git add tests/cross_stack/r29_integration_test.go
git commit -m "test(cross_stack): add 1000-block BeginBlocker/EndBlocker ordering stress test"
```

---

### Task 5: TestGenesisExportImport_WithR29State

**Files:**
- Modify: `tests/cross_stack/r29_integration_test.go`

**Step 1: Add the genesis round-trip test**

Append to `r29_integration_test.go`:

```go
// TestGenesisExportImport_WithR29State verifies that R29 state survives
// a genesis export/import round-trip.
func TestGenesisExportImport_WithR29State(t *testing.T) {
	// ── Phase 1: Boot first app and accumulate R29 state ─────────────────

	h := NewTestHarness(t)
	domain := "genesis-roundtrip"

	// Seed R29-1: Domain stats.
	h.KnowledgeKeeper.SetDomainStats(h.Ctx, &knowledgekeeper.DomainStats{
		Domain:      domain,
		ActiveCount: 800,
		AtRiskCount: 50,
	...
	// Seed R29-2: Epistemic state.
	require.NoError(t, h.KnowledgeKeeper.SetDomainEpistemicState(h.Ctx, &knowledgetypes.DomainEpistemicState{
		Domain:           domain,
		Temperature:      350_000,
		ConformityStreak: 3,
		VindicationCount: 2,
	...
	// Seed R29-3: Role record.
	require.NoError(t, h.KnowledgeKeeper.SetDomainRoleRecord(h.Ctx, &knowledgetypes.DomainRoleRecord{
		Domain:              domain,
	...
	// Seed R29-4: Correction outcomes.
	for i := 0; i < 5; i++ {
		h.AlignmentKeeper.SetCorrectionOutcome(h.Ctx, &aligntypes.CorrectionOutcome{
			Height:      uint64(i + 1),
			Dimension:   aligntypes.DimKnowledgeQuality,
	...
	// Enable alignment and autopoiesis.
	h.AlignmentKeeper.SetState(h.Ctx, &aligntypes.AlignmentState{
		Enabled:              true,
		PreviousCategory:     aligntypes.CategoryDegraded,
		LastObservationHeight: 0,
	...
	// Advance 100 blocks to accumulate EndBlocker-generated state.
	h.AdvanceBlocks(100)

	// ── Phase 2: Export genesis ──────────────────────────────────────────

	exported, err := h.App.ExportAppStateAndValidators(false, nil, nil)
	require.NoError(t, err)
	...
	_, err = app2.InitChain(&abci.RequestInitChain{
		...
		ConsensusParams: simtestutil.NewAppOptionsWithFlagHome(t.TempDir()),
	})
	// Note: InitChain with exported genesis may require consensus params.
	// If this fails, it's because the exported state doesn't include the
	// validator set properly. Fall back to using DefaultConsensusParams.
	if err != nil {
		_, err = app2.InitChain(&abci.RequestInitChain{
			ChainId:         chainID2,
			AppStateBytes:   exported.AppState,
	...
	// ── Phase 4: Verify R29 state survived the round-trip ────────────────

	// R29-1: Domain stats.
	stats, found := app2.KnowledgeKeeper.GetDomainStats(ctx2, domain)
	require.True(t, found, "domain stats must survive export/import")
	require.Equal(t, uint64(800), stats.ActiveCount)

	// R29-2: Epistemic state.
	epState, found, err := app2.KnowledgeKeeper.GetDomainEpistemicState(ctx2, domain)
	require.NoError(t, err)
	require.True(t, found, "epistemic state must survive export/import")
	require.Greater(t, epState.Temperature, uint64(0), "temperature must be preserved")

	// R29-3: Role record.
	roleRec, found := app2.KnowledgeKeeper.GetDomainRoleRecord(ctx2, domain)
	require.True(t, found, "role record must survive export/import")
	require.Greater(t, roleRec.AgentCorrectCalls, uint64(0))

	// R29-5: Capture metrics.
	metrics, found := app2.CaptureDefenseKeeper.GetCaptureMetrics(ctx2, domain)
	require.True(t, found, "capture metrics must survive export/import")
	require.True(t, metrics.Flagged, "capture flag must survive export/import")

	// ── Phase 5: Run 10 more blocks on the imported app ──────────────────

	height := app2.LastBlockHeight()
	require.NotPanics(t, func() {
		for i := 0; i < 10; i++ {
			height++
			blockCtx := app2.NewContext(true).WithBlockHeight(height).WithChainID(chainID2)
			app2.BeginBlocker(blockCtx)
			app2.EndBlocker(blockCtx)
		}
	})
}
```

**Step 2: Check compilation and imports**

The genesis test uses `abci` and `simtestutil` which are already imported in `harness_test.go` (same package). Verify with:
...
Run: `go test ./tests/cross_stack/... -run TestGenesisExportImport_WithR29State -v -count=1 -timeout 120s`
Expected: PASS

Note: The genesis export/import may expose issues with modules that don't properly implement ExportGenesis for R29 state (e.g., knowledge DomainStats, epistemic state, role records stored as JSON in KV store may not be included in the genesis export). If so, the test will fail on Phase 4 assertions — this is a legitimate finding that should be reported as a discovered gap.

**Step 4: Commit**

```bash
git add tests/cross_stack/r29_integration_test.go
git commit -m "test(cross_stack): add genesis export/import round-trip test for R29 state"
```
...
**Step 1: Run all cross-stack tests**

Run: `go test ./tests/cross_stack/... -v -count=1 -timeout 600s`
Expected: ALL PASS
...
Only if adjustments were needed during the run.
